...
<head>
	...
	<script>
		/* Flag that JS is on so the no-JS fallback shows all content. motion.js
		   adds the `almanac-motion` class (which arms the reveal start-states)
...
<body <?php body_class(); ?>>
<?php wp_body_open(); ?>
<a class="skip-link screen-reader-text" href="#content"><?php esc_html_e( 'Skip to content', 'almanac' ); ?></a>
...
<header class="site-header">
	<div class="site-wrap site-header__inner">
		<div class="site-branding">
			...
			<?php else : ?>
				<?php almanac_leaf_mark(); ?>
			<?php endif; ?>
			<div class="site-branding__text">
				<?php
				$almanac_title_tag = ( is_front_page() && is_home() ) ? 'h1' : 'p';
				printf(
					'<%1$s class="site-title"><a href="%2$s" rel="home">%3$s</a></%1$s>',
					esc_attr( $almanac_title_tag ),
					esc_url( home_url( '/' ) ),
					esc_html( get_bloginfo( 'name' ) )
				);
				?>
				<?php
				$almanac_desc = get_bloginfo( 'description', 'display' );
				if ( $almanac_desc || is_customize_preview() ) :
					?>
					<p class="site-description"><?php echo esc_html( $almanac_desc ); ?></p>
				<?php endif; ?>
			</div>
		</div>
